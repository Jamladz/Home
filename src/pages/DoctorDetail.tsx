...
export default function DoctorDetail() {
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    // Check localStorage for spam prevention
    const historyKey = "dawini_bookings";
    let bookingHistory: { date: string; specialty: string }[] = [];
    try {
      const stored = localStorage.getItem(historyKey);
      if (stored) bookingHistory = JSON.parse(stored);
    } catch(e) {}

    const hasBookedTodayThisSpecialty = bookingHistory.some(b => b.date === finalDate && b.specialty === doctor.specialty);
    
    if (hasBookedTodayThisSpecialty) {
      alert(tx(
        "لا يمكنك حجز أكثر من موعد واحد في نفس التخصص خلال نفس اليوم.",
        "Vous ne pouvez pas prendre plus d'un rendez-vous dans la même spécialité le même jour.",
        "You cannot book more than one appointment in the same specialty on the same day."
      ));
      return;
    }
    ...
    try {
      ...
      bookingHistory.push({ date: finalDate, specialty: doctor.specialty });
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 sm:p-8 overflow-y-auto text-sm text-slate-600 flex-1 relative custom-scrollbar bg-slate-50/30">
              {tx((<div className="space-y-8">
                ...
                <p className="text-sm leading-relaxed text-slate-600 bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50">
                  These conditions represent a binding agreement between the user of the Dawini service and the platform. By using the platform, you agree to be bound by these terms and conditions.
                </p>
                <section className="space-y-3">
                  <h4 className="font-bold text-slate-800 text-base flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600">
                  ...
                  <div className="pl-11 pr-4 space-y-2">
                    <p className="leading-relaxed">
                      Dawini is a digital intermediation platform aimed at facilitating the connection between patients and healthcare professionals.
                    </p>
                    <ul className="list-disc pl-5 space-y-1 text-slate-500 marker:text-slate-300">
                      <li>The platform does not provide medical consultations, diagnoses, or treatments.</li>
                      <li>The information available on the platform is for guidance only and does not in any way replace a specialized medical visit.</li>
                    </ul>
                  </div>
                </section>
                <section className="space-y-3">
                  <h4 className="font-bold text-slate-800 text-base flex items-center gap-3">
                    ...
                    2. Appointment Booking Policy
                  </h4>
                  <div className="pl-11 pr-4 space-y-2">
                     <ul className="list-disc pl-5 space-y-2 text-slate-500 marker:text-slate-300">
                      <li>Appointments booked via the platform are preliminary organizational appointments and are subject to the clinic's circumstances.</li>
                      <li>The patient agrees to <strong className="text-slate-700">arrive at least 15 minutes before</strong> the appointment to confirm their arrival.</li>
                      <li>The platform is not responsible for delays caused by emergencies at the clinic which may alter the order of patients.</li>
                    </ul>
                  </div>
                </section>
                <section className="space-y-3">
                  ...
                  <div className="pl-11 pr-4 space-y-2">
                    <p className="leading-relaxed text-slate-600">
                      We take your privacy very seriously. Data is collected securely for a single purpose.
                    </p>
                    <ul className="list-disc pl-5 space-y-2 text-slate-500 marker:text-slate-300">
                      <li>The platform collects essential data (name, phone number) exclusively for managing the medical booking.</li>
                      <li>We adhere to the highest encryption and security standards to protect your information.</li>
                      <li>Your personal data will <strong>never be sold, rented, or shared</strong> with any third parties or advertising companies.</li>
                    </ul>
                  </div>
                </section>
                <section className="space-y-3">
                  <h4 className="font-bold text-slate-800 text-base flex items-center gap-3">
                    ...
                    4. Fair Use and Prohibitions
                  </h4>
                  <div className="pl-11 pr-4 space-y-2">
                    <p className="leading-relaxed text-slate-600">
                      Fake or repeated bookings for the purpose of sabotage are prohibited. 
                      The platform has the right to track IP addresses and block numbers proven to engage in suspicious activity without prior notice.
                    </p>
                  </div>
                </section>
                <section className="space-y-3">
                  ...
                  <div className="pl-11 pr-4 space-y-2">
                     <p className="leading-relaxed text-slate-600">
                      The platform strictly disclaims any medical complications, diagnostic errors, or disputes arising between the patient and the doctor. The medical and contractual relationship is exclusively between the patient and the healthcare provider.
                    </p>
                  </div>
                </section>
              </div>))}

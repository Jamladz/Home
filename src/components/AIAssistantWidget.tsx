...
import { MessageSquare, X, Send, Bot, Loader2, Mic, MicOff, Volume2, VolumeX, ImageIcon } from "lucide-react";
...
export function AIAssistantWidget() {
      ...
      if (SpeechRecognition) {
        ...
        recognition.onerror = (event: any) => {
          ...
          if (event.error === 'not-allowed') {
            alert(language === "ar" ? "الرجاء السماح باستخدام الميكروفون للتحدث." : "Veuillez autoriser l'accès au microphone pour parler.");
          }
        };
  ...
  const toggleRecording = () => {
    if (isRecording) {
      ...
      if (recognition) {
        setInput(""); // Clear previous input before recording
        usedVoiceRef.current = true;
        recognition.lang = language === "ar" ? "ar-DZ" : "fr-FR";
        try {
          recognition.start();
          setIsRecording(true);
        } catch (e) {
        ...
        alert(language === "ar" ? "التعرف على الصوت غير مدعوم في متصفحك." : "Reconnaissance vocale non supportée par votre navigateur.");
      }
    }
  };
  ...
  const speakText = (text: string) => {
    ...
    utterance.lang = language === "ar" ? "ar-DZ" : "fr-FR";
    utterance.rate = 1.0;
    
    // Try to find an appropriate voice
    const voices = window.speechSynthesis.getVoices();
    const appropriateVoice = voices.find(v => v.lang.startsWith(language === "ar" ? "ar" : "fr"));
    if (appropriateVoice) {
      ...
      setMessages([
        {
          ...
          text:
            language === "ar"
              ? "مرحباً بك! أنا مساعدك الطبي الذكي. واش نقدر نعاونك اليوم بخصوص صحتك؟ يمكنك التحدث معي بضغطة زر الميكروفون."
              : "Bonjour ! Je suis votre assistant médical intelligent. Comment puis-je vous aider aujourd'hui ? Vous pouvez me parler en utilisant le microphone.",
        },
      ]);
    }
  }, [language]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    window.speechSynthesis?.cancel(); // Stop talking when user sends a new message
    
    const userText = input.trim();
    const currentImage = selectedImage;
    const currentFile = imageFile;
    const shouldSpeakResponse = usedVoiceRef.current;
...
Assistant:`;
      ...
      if (response.text) {
        const textToSpeak = response.text.replace(/[\*\#\_]/g, '');
        setMessages((prev) => [
          ...prev,
          ...
          >
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 text-white p-4 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-indigo-100 text-[10px] flex items-center">
                    ...
                    {language === "ar" ? "صوت ونص متصل" : "En ligne (Voix activée)"}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`text-indigo-100 hover:text-white hover:bg-white/10 p-1.5 rounded-full transition ${!voiceEnabled ? 'opacity-50' : ''}`}
            ...
            >
              <div className="flex flex-col gap-2">
                {isRecording && (
                  <div className="flex items-center justify-center gap-2 text-xs text-red-500 py-1 font-medium animate-pulse">
                    <div className="w-2 h-2 rounded-full bg-red-500"></div>
                    {language === "ar" ? "جاري الاستماع... تحدث الآن" : "Écoute en cours..."}
                  </div>
                )}
                ...
                <div className={`flex items-end gap-2 bg-slate-50 border rounded-2xl px-2 py-2 transition-all ${isRecording ? 'border-red-300 ring-2 ring-red-100' : 'border-slate-200 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100'}`}>
                  <textarea
                    value={input}
                    onChange={(e) => {
                    ...
                    placeholder={
                      isRecording 
                        ? (language === "ar" ? "تحدث..." : "Parlez...") 
                        : (language === "ar" ? "أرفق صورة، اكتب أو تحدث..." : "Joignez une image, écrivez ou parlez...")
                    }
                  ...
                  <div className="flex gap-1 items-center shrink-0">
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-xl transition ${isRecording ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'text-slate-500 hover:bg-slate-100 hover:text-indigo-600'}`}
                      title={language === "ar" ? "تحدث" : "Parler"}
                    >
